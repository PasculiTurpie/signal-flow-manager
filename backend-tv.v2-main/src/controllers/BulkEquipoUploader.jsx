...
const BulkEquipoUploader = () => {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    setLoading(true);
    try {
      const data = await api.validateExcelEquipos(f);
      setPreview(data);
      setStep(2);
    } catch (error) {
  ...
  const handleConfirm = async () => {
    setLoading(true);
    try {
      const data = await api.bulkCreateEquipos(file);
      setResults(data);
      setStep(3);
    } catch (error) {
  ...
  return (
    <div className="outlet-main" style={{ maxWidth: 800, margin: "0 auto", padding: 20 }}>
      ...
      <p style={{ color: "#666" }}>
        Sube un Excel con columnas <code>nombre</code>, <code>marca</code>, <code>modelo</code>,{" "}
        <code>tipoNombre</code> (debe existir ya en TipoEquipo), y opcionalmente{" "}
        <code>ip_gestion</code>, <code>satelliteNombre</code>, <code>irdNombre</code>. Si ya existe
        un equipo con el mismo nombre+modelo, se actualiza en vez de duplicarse.
      </p>
      ...
      {step === 2 && preview && (
        <div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
            ...
            >
              {loading ? "Procesando..." : `Confirmar y cargar ${preview.totalRows} equipo(s)`}
            </button>
          </div>
        </div>
      )}
              ...
              >
                {results.data.duplicadosEnArchivo.map((d, i) => (
                  <div key={i} style={{ padding: 8, borderBottom: "1px solid #ffe082" }}>
                    <strong>{d.nombre}</strong> (modelo {d.modelo}) aparece en las filas: {d.filas.join(", ")}
                  </div>
                ))}
              </div>
            </div>
...
export default BulkEquipoUploader;
