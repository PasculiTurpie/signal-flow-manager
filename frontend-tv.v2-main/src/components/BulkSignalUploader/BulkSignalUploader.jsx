...
const BulkSignalUploader = () => {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    setLoading(true);
    try {
      const data = await api.validateExcelSignals(f);
      setPreview(data);
      setStep(2);
    } catch (error) {
  ...
  const handleConfirm = async () => {
    setLoading(true);
    try {
      const data = await api.bulkCreateSignals(file);
      setResults(data);
      setStep(3);
    } catch (error) {
  ...
  return (
    <div className="outlet-main" style={{ maxWidth: 800, margin: "0 auto", padding: 20 }}>
      <h2>Carga Masiva de Señales</h2>
      <p style={{ color: "#666" }}>
        Sube un Excel con columnas <code>nameChannel</code>, <code>numberChannelSur</code>,{" "}
        <code>numberChannelCn</code>, <code>logoChannel</code>, <code>severidadChannel</code>,{" "}
        <code>tipoServicio</code>, <code>tipoTecnologia</code>, y opcionalmente <code>source</code>,{" "}
        <code>contactos</code> (nombres separados por coma). Si ya existe una señal con el mismo{" "}
        <code>nameChannel</code>+<code>tipoTecnologia</code>, se actualiza en vez de duplicarse.
      </p>
      ...
      {step === 2 && preview && (
        <div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
            ...
            >
              {loading ? "Procesando..." : `Confirmar y cargar ${preview.totalRows} señal(es)`}
            </button>
          </div>
        </div>
      )}
          ...
          <div style={{ display: "flex", gap: 16, margin: "16px 0" }}>
            <div style={{ ...okBox, flex: 1, textAlign: "center" }}>
              <div style={{ fontSize: 28, fontWeight: "bold", color: "#388e3c" }}>
                {results.data?.summary?.signalsCreated || 0}
              </div>
              <div>Creados</div>
            </div>
            <div style={{ ...box, flex: 1, textAlign: "center", backgroundColor: "#f5f5f5" }}>
              <div style={{ fontSize: 28, fontWeight: "bold" }}>
                {results.data?.summary?.signalsUpdated || 0}
              </div>
              <div>Actualizados (ya existían)</div>
            </div>
              ...
              >
                {results.data.duplicadosEnArchivo.map((d, i) => (
                  <div key={i} style={{ padding: 8, borderBottom: "1px solid #ffe082" }}>
                    <strong>{d.nameChannel}</strong> (tecnología {d.tipoTecnologia}) aparece en las filas: {d.filas.join(", ")}
                  </div>
                ))}
              </div>
            </div>
...
export default BulkSignalUploader;
